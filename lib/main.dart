...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> _applyToJob(JobListing job, {String? coverLetter}) async {
    if (_hasApplied(job.id)) {
      ...
      if (existing != null) {
        final daysSince = DateTime.now().difference(existing.appliedAt).inDays;
        if (daysSince < job.reapplyWindowDays) {
          final appliedDateStr = _formatYmd(existing.appliedAt.toLocal());
          final canReapplyDate = existing.appliedAt.add(
            Duration(days: job.reapplyWindowDays),
          );
          final canReapplyStr = _formatYmd(canReapplyDate.toLocal());
          if (!mounted) return;
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isAutoRejected =
            app.status == 'rejected' && hasFeedback &&
            appFeedback.message.contains('minimum match threshold');
